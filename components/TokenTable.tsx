...
const TokenTable = ({ tokenData }: Props) => {
  const [tokenDataSorted, setTokenDataSorted] = useState([]);
  useEffect(() => {
    let _tokenData = tokenData.sort((a, b) =>
      (a.amount / 10 ** a.decimals) * a.priceUSD <
      (b.amount / 10 ** b.decimals) * b.priceUSD
        ? 1
        : (b.amount / 10 ** b.decimals) * b.priceUSD <
          (a.amount / 10 ** a.decimals) * a.priceUSD
        ? -1
    ...
    setTokenDataSorted(_tokenData);
  }, [tokenData]);
  return (
    <div>
      {tokenDataSorted.length > 0 && (
        <div className="p-4">
